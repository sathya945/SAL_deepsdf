...
def load_data(filename="00239.obj"):
...
class pDataset(Dataset):
    ...
    def __init__(self):
        ...
        self.sigmas = self.ptree.query(self.points, np.int(30.0))[0][:,-1]
    ...
    def __getitem__(self, index):
        if NETWORK == 0 :
            rand_idx = np.random.choice(self.points.shape[0],32)
...
for e in range(epochs):
    running_loss = 0
    count = 0
    for data,gt_usdf in trainloader:
        data = data.to(device)
    ...
    if e%100 ==0 and e>0:
        to_mesh(net,device,"80testing"+str(e))
